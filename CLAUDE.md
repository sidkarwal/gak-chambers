...
- **HTTPS cert issued + enforced (2026-09-25).** The cert got genuinely stuck in GitHub's provisioning queue for a while (DNS was correct the whole time — confirmed repeatedly via direct-IP checks showing the default `*.github.io` cert on all 4 edge IPs). Fix: `gh api -X PUT repos/sidkarwal/gak-chambers/pages -f "cname="` (clear) then `-f "cname=gakchambers.com"` (re-set) forced GitHub to restart verification from scratch — cert was approved within ~1 minute. Then `gh api -X PUT repos/sidkarwal/gak-chambers/pages -F "https_enforced=true"` (note: `-F` not `-f`, since `-f` sends a string and the endpoint requires a real boolean). Cert covers both `gakchambers.com` and `www.gakchambers.com`, expires 2026-12-24. Migration is now fully complete.
